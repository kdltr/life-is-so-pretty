...
(define (make-dream-game-state)
  ...
  (let* ((good-objects (interact *user-choices*))
    ...
    (let ((factor (/ (max life 0) max-life)))
      ...
      (color-mod-all-textures! (make-list 3 (round (+ 100 (* factor 155))))))
    (rec (state dt)
             ...
             (if (null? (cdr items))
                 (begin (reset-movement!)
                        (cond ((= good-objects num-choices)
                               ...
                               (color-mod-all-textures! '(255 255 255))
                               (make-last-action-state 'good door-texture good-end-phrases good-end-screen-state))
                              ((<= life 0)
                               (make-last-action-state 'bad window-texture bad-end-phrases bad-end-screen-state))
                              (else
                               (set! *user-choices* '())
                               default-game-state)))
                 (begin
...
(define (make-last-action-state ending clickable-texture texts next-state)
  (set! *users-choices* '())
  (rec (state dt)
       (move-player! dt)
       (show-scene! ending)
       (show-player!)
       (let* ((flr (find-filler-at-player))
              (obj (find-object-at-player))
              (tex (and flr (scene-filler-texture flr))))
         (cond ((and *player-interacting*
                     (eq? tex clickable-texture))
                (reset-movement!)
                next-state)
               ((and *player-interacting*
                     obj)
                (let ((phrase (random-elt texts)))
                  (reset-movement!)
                  (rec (substate dt)
                       (show-scene! ending)
                       (show-player!)
